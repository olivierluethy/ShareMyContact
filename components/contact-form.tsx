...
import { trackFormEvent, trackFormStartedOnce } from "@/lib/smc-tracking"
...
export function ContactForm() {
  ...
  function generateUrl(): boolean {
    ...
    void trackFormEvent("form_submitted", {
      has_phone:    formData.phone.trim().length    > 0,
      has_email:    formData.email.trim().length    > 0,
      has_company:  formData.company.trim().length  > 0,
      has_website:  formData.website.trim().length  > 0,
      has_linkedin: formData.linkedin.trim().length > 0,
    })
